...
def minimum_arg(x: xr.DataArray, *, dim="tas_bin", lmmt=10.0, ummt=30.0):
    ...
    x_in_range = x.where(in_range)
    constrained_arg = x_in_range.argmin(dim=dim)
    mmt_temp = x_in_range.idxmin(dim=dim)
    on_bound = (mmt_temp < 11) | (mmt_temp > 29)

    # Fallback: local minimum along tas_bin, per (sample, age-cohort)
    ...
    # Get the minima values (without lazy indexing)
    positions = xr.DataArray(np.arange(x.sizes[dim]), dims=dim)
    min_value = x.where(positions == min_arg).min(dim=dim)
...
def calculate_beta(ds: xr.Dataset) -> xr.Dataset:
    ...
    beta, mmt_idx = _calculate_shifted_baseline_beta(ds, baseline_time=2025)
    ...
    positions = xr.DataArray(np.arange(beta.sizes["tas_bin"]), dims="tas_bin")
    mmt = beta["tas_bin"].where(positions == mmt_idx).max("tas_bin")
